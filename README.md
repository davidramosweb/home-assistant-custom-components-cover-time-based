...
Optional settings:
- `send_stop_at_ends` defaults to False. If set to `True`, the Stop script will be run after the cover reaches to 0 or 100 (closes or opens completely). This is for people who use interlocked relays in the scripts to drive the covers, which need to released them when the covers reach the end positions.
